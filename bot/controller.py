...
def _head_lock_path(project_id: str, project_path: Path) -> Path:
    """head worktree 의 HEAD_LOCK 경로.
    head_wt = <project parent>/<project_id>-wt-head/coordination/HEAD_LOCK
    """
    head_wt = project_path.parent / f"{project_id}-wt-head"
    return head_wt / "coordination" / "HEAD_LOCK"
...
def _plan_path(project_id: str, project_path: Path, inbox_id: str) -> Path:
    """head worktree 의 plans/<inbox-id>.md 경로."""
    head_wt = project_path.parent / f"{project_id}-wt-head"
    return head_wt / "coordination" / "plans" / f"{inbox_id}.md"
...
async def _post_spawn_orchestration(
    ...
    appeared = await _wait_for_file_state(head_lock, want_exists=True, timeout=180, poll=5)
    if not appeared:
        try:
            await target_channel.send(
                f"⚠ `{project_id}` head 세션이 시작 안 됨 (3분 타임아웃). 수동 확인 필요."
            )
        except Exception:
            pass
        return
    ...
    try:
        cleared = await _wait_for_file_state(head_lock, want_exists=False, timeout=3600, poll=15)
    finally:
    ...
    if not cleared:
        try:
            await target_channel.send(
                f"⚠ `{project_id}` head 세션이 60분 내 완료 안 됨. 수동 확인 필요."
            )
        except Exception:
            pass
        return
    ...
    try:
        await target_channel.send(
            f"✅ `{project_id}` head 완료 감지 — verdict 확인 중"
        )
    except Exception:
        pass
...
def _find_review_path(
    ...
    for base in (head_wt, project_path):
        candidate = base / "coordination" / "review-inbox" / review_filename
        if candidate.exists():
            return candidate
    return None
...
async def cmd_main_merge(ctx: commands.Context, project_id: str | None = None) -> None:
    """v0.27: work_branch → stable_branch (main) 머지.

    안전장치:
    1. Dry-run (temp 브랜치) 로 충돌 사전 감지
    2. 충돌 없으면 → 실제 머지 + push + rich embed
    3. 충돌 있으면 → 파일 목록 embed 포스트 + 중단 (stable 안 건드림)

    실패 시 사용자가 수동으로 해결 — 봇은 자동 resolve 시도 안 함.
...
def _build_help_embed() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "**`@bot main-merge [project-id]`** — v0.27: work_branch → stable_branch 머지 (dry-run 충돌 감지 + 안전 중단)\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "**`@bot usage [days=7]`** — Claude CLI 실 비용 (ccusage)"
        ),
        inline=False,
    )
...
def _gather_project_state(
    ...
    head_review_dir = project_path.parent / f"{project_id}-wt-head" / "coordination" / "review-inbox"
