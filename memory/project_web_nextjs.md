...
**데이터 파이프라인:** `tools/SenaDataExport/`(net8.0 콘솔)가 DB/Models 소스를 직접 Compile Include(WPF 의존성 없음)하여 `CharacterDb.Characters`를 characters.json으로 직렬화. 데이터 갱신 시 `cd tools/SenaDataExport && dotnet run` 재실행.
