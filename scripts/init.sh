...
# === .env.local (PROJECT_ROOT 기준 — subdir 모드에선 .coord/ 밖) ===
say ".env.local 점검..."
...
# subdir 모드면 coordination 런타임 경로는 .coord/ prefix
if [[ "$INSTALL_MODE" == "subdir" ]]; then
  GITIGNORE_ENTRIES=(
...
# subdir 모드: 템플릿은 .coord/.claude 에 있음, PROJECT_ROOT/.claude 로 복사
...
if [[ "$INSTALL_MODE" == "subdir" ]] && [[ -d "$CLAUDE_SRC" ]] && [[ ! -d "$CLAUDE_DST" ]]; then
  say "  .coord/.claude → $CLAUDE_DST 재동기화 (subdir 모드)"
  mkdir -p "$CLAUDE_DST"
