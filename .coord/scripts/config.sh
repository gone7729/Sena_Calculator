...
if [[ -z "${_COORD_ROOT_DETECTED:-}" ]]; then
  ...
  # v0.39+: .env.local 탐색 — PROJECT_ROOT/.env.local 우선 (관습 + 단일 source of truth).
  # .coord/.env.local 은 opt-in isolation (프로젝트가 자체 .env.local 를 쓸 때만).
  # v0.38 의 ".coord/ 우선" 전략은 Windows Git Bash cp fallback → stale 문제로 반전.
  # .coord/.env.local 쓰려면 scripts/link-env-to-coord.sh 로 symlink 생성 (cp 는 금지).
  if [[ -f "$PROJECT_ROOT/.env.local" ]]; then
    ENV_FILE_RESOLVED="$PROJECT_ROOT/.env.local"
  elif [[ -f "$COORD_ROOT/.env.local" ]]; then
    ENV_FILE_RESOLVED="$COORD_ROOT/.env.local"
  else
    ENV_FILE_RESOLVED="$PROJECT_ROOT/.env.local"
  fi

  export COORD_ROOT PROJECT_ROOT INSTALL_MODE ENV_FILE_RESOLVED
