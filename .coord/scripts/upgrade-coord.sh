...
# === 9.5 .gitignore 동기화 (v0.41+: runtime + secrets 만 ignore, 통째 ignore 반전) ===
# v0.37~v0.40 은 .coord/ / .claude/ 통째 ignore 였으나 workflow (inbox-send 등) 이
# git sync 에 의존하므로 통째 ignore 시 동작 불능 → 개별 파일만 ignore 로 반전.
say "9.5/10 .gitignore 동기화 (v0.41+ runtime + secrets only)..."
if [[ "$INSTALL_MODE" == "subdir" ]]; then
  PROJECT_ROOT_GI="$(cd "$COORD_ROOT/.." && pwd)"
  GITIGNORE_FILE="$PROJECT_ROOT_GI/.gitignore"
  GITIGNORE_ENTRIES=(
  ...
  # v0.41+: 이전 버전이 남긴 잘못된 항목 제거 (.coord/ 통째, .claude/ 통째)
  STALE_ENTRIES=(".coord/" ".claude/")
  STALE_REMOVED=0
  for stale in "${STALE_ENTRIES[@]}"; do
    if grep -qxF "$stale" "$GITIGNORE_FILE" 2>/dev/null; then
      if [[ "$DRY_RUN" -eq 1 ]]; then
        warn "  [dry-run] stale entry 제거 예정: $stale"
      else
        # macOS/Linux 호환 sed (in-place 백업)
        sed -i.bak "\|^${stale}\$|d" "$GITIGNORE_FILE"
        rm -f "$GITIGNORE_FILE.bak"
      fi
      STALE_REMOVED=$((STALE_REMOVED+1))
    fi
  done
  [[ $STALE_REMOVED -gt 0 ]] && warn "  v0.37~v0.40 stale entry $STALE_REMOVED 개 제거 (workflow 복원)"
  ...
  if [[ $GI_ADDED -eq 0 ]] && [[ $STALE_REMOVED -eq 0 ]]; then
    ok "  .gitignore 이미 최신"
  else
    [[ "$DRY_RUN" -eq 1 ]] && ok "  $GI_ADDED 항목 추가 예정" || ok "  $GI_ADDED 항목 추가 → $GITIGNORE_FILE"
  fi

  # v0.41+ export-ignore / v0.46+ merge=ours
  GA_FILE="$PROJECT_ROOT_GI/.gitattributes"
  [[ ! -f "$GA_FILE" ]] && [[ "$DRY_RUN" -eq 0 ]] && touch "$GA_FILE"
  GA_ENTRIES=(
    ".coord/ export-ignore"
    ".coord/** merge=ours"
    ".claude/** merge=ours"
  ...
  for entry in "${GA_ENTRIES[@]}"; do
    if ! grep -qxF "$entry" "$GA_FILE" 2>/dev/null; then
      if [[ "$DRY_RUN" -eq 1 ]]; then
        warn "  [dry-run] .gitattributes += $entry"
      else
        echo "$entry" >> "$GA_FILE"
      fi
      GA_ADDED=$((GA_ADDED+1))
  ...
  if [[ $GA_ADDED -gt 0 ]]; then
    [[ "$DRY_RUN" -eq 1 ]] && ok "  .gitattributes $GA_ADDED 항목 추가 예정" || \
      ok "  .gitattributes $GA_ADDED 항목 추가 (export-ignore + merge=ours, v0.46)"
  fi
else
  ok "  flat 모드 — .gitignore 변경 없음"
fi
