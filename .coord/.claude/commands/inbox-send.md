...
# v0.43+: subdir 모드 대응 — head 도 .coord/ 하위에 coordination 이 있음
HEAD_WT="$(dirname "$PROJECT_ROOT")/${PROJECT_NAME}-wt-head"
if [[ -d "$HEAD_WT/.coord/coordination" ]]; then
  LOCK_FILE="$HEAD_WT/.coord/coordination/HEAD_LOCK"
else
  LOCK_FILE="$HEAD_WT/coordination/HEAD_LOCK"
fi

# v0.45: HEAD_LOCK atomic acquire — set -o noclobber 로 check-then-create race 제거.
# 동시 @bot send 2건이 들어와도 한 쪽만 lock 획득, 나머지는 즉시 거부.
...
결과의 verdict (go / needs-fix / block) 가 review-inbox 파일 헤더에 기록되어야 이번 세션 종료 가능. 실패 시에도 verdict: block 또는 verdict: pending 을 명시 기록하고 종료."
  ...
  (
    ...
    claude -p "$HEAD_SPAWN_PROMPT" \
      ...
      --model "${HEAD_MODEL:-claude-opus-4-7}" \
      --output-format json \
      > "/tmp/head-auto-$(date +%s).log" 2>&1
    rm -f "$LOCK_FILE"  # 종료 시 락 해제 (v0.43+: subdir/flat 동일)
  ) &
