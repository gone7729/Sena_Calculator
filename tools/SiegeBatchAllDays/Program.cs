...
foreach (var (day, ids) in DAYS)
{
    ...
    // [좌표상승] 인자 "좌표상승": 기어↔로테/진형 2패스 탐색 — 기어 선정 레짐을 최종 빔 레짐과 정합.
    //   ON이면 딜러 메인옵을 사람이 박지 않아도 시뮬이 스스로 찾으므로, 검증을 위해 라이언 메인옵 강제도 해제.
    bool coordAscent = args.Contains("좌표상승");
    if (coordAscent) cfg.CoordinateAscentGear = true;
    if (!args.Contains("라이언약확") && !coordAscent && (day == "목요일" || day == "금요일"))
